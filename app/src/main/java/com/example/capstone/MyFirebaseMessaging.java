...
public class MyFirebaseMessaging extends FirebaseMessagingService {
    ...
    public MyFirebaseMessaging() {
        super();
        Task<String> token1 = FirebaseMessaging.getInstance().getToken();
        token1.addOnCompleteListener(new OnCompleteListener<String>() {
            @Override
    ...
    private RemoteViews getCustomDesign(String title, String message) {
        RemoteViews remoteViews = new RemoteViews(getApplicationContext().getPackageName(), R.layout.popup_message);
        remoteViews.setTextViewText(R.id.messageTitle, title);
        remoteViews.setTextViewText(R.id.messageContent, message);
        remoteViews.setImageViewResource(R.id.logo, R.drawable.petsafe_logo);
        return remoteViews;
    }
    ...
    public void showNotification(String title, String message) {
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(getApplicationContext(), channel_id)
                .setSmallIcon(R.drawable.petsafe_logo)
                .setSound(uri)
                .setAutoCancel(true)
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) { //안드로이드 버전이 커스텀 알림을 불러올 수 있는 버전이면
            ...
            builder = builder.setContentTitle(title)
                    ...
                    .setSmallIcon(R.drawable.petsafe_logo); //커스텀 레이아웃에 사용된 로고 파일과 동일하게..
        }
